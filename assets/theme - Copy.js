...
alert('Sort by Changed');
      ...
      value: function _reloadProducts() {
        ...
        var fetchAjaxURL = "";
        // Genius Minds - Modified Theme to Load filter results via AJAX versus FullPage reloads
        if(location.pathname.indexOf('get-started') > -1) {
          var tags = this.currentTags.length > 0 ? this.currentTags.join('+') : '';
          fetchAjaxURL = this.settings['collectionUrl'] + '/' + tags + '?view=ajax&sort_by=' + this.currentSortBy;
          alert('Within IF - ' + fetchAjaxURL);
          ...
          if (history.replaceState) {
              ...
              fetchAjaxURL = location.pathname + '?view=ajax&sort_by=' + this.currentSortBy;
